...
\personal{My contributions are related to the study of differential GPS. I ran tests to understand its functioning and the meaning of the output data. I also analysed the noise of this system, which will be used to choose the best configuration to install at the observation site, as presented in section~\ref{section: gps}. Then, I wrote a software adapted to our specific use that is detailed in Section~\ref{section: calsource pos & orientation}. This software can be found in the qubicsoft GitHub repository : \url{https://github.com/qubicsoft/qubic/blob/master/qubic/lib/Qgps.py}. \tom{The installation of the antennas on the calibration tower were decided based on the results of the analysis presented in this chapter}.}
...
\steve{Marie-Anne's} work was done thirteen years ago, a more up-to-date analysis, taking into account the evolution of the instrumental design and of our understanding, is needed to confirm these predictions. \tom{A PhD student recently started to work on this vast topic.}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{patch_visible_time.pdf}
    \caption{Plot showing the visible time of the targeted QUBIC patch during one sidereal day from the instrument point of view. This plot includes the instrumental limitation of observing between [30, 70]° to keep the maximal efficiency of the cryogenic system. In addition, we added the width of the QUBIC beam ($\sim$ 13°) in order to compute the maximum time of visibility of the patch. \rmk{Is this plot understandable ?}
    \steve{\enquote{Instead of using the beam width, maybe you should use the separation
to the secondary peak of the synthesised beam.} Distance between peaks depend on frequency. If we chose distance at 150 GHz, it corresponds approximately at the beam hald-width.}}
...
\end{equation}
where $d_{\text{cal}}$ is the distance between the calibration source and QUBIC. As said in the Section~\ref{cal tower}, the calibration source will be placed at 48.6 m height, the distance on the ground between the tower and QUBIC is 38.44 m while the height of QUBIC from the ground is approximatively 4 m. By simple trigonometry, we can see that $d_{\text{cal}}= 58.88$ m from these values.
...
The detailed characteristics of the calibration source have been discussed in the Section~\ref{section: calsource characteristics}. For this work, we use some simplifying hypothesis : we consider that the calibration source's beam is perfectly Gaussian (which is a reasonable assumption at long distance, where the source can be considered as a point source), it emits at 150 GHz, with a Full Width at Half Maximum (FHWM) of $13^\circ$. \steve{\enquote{The beam from the calibration source really is close to a Gaussian. The horn is designed with the corrugations for that purpose. A point source in the far field looks like an isotropic source, not a Gaussian beam.  The important point is that the wavefront that arrives at the telescope window is a plane wave.  When the calibration source is far enough away, we only detect a small portion of its main lobe which appears like a flat wave front (in the same way that some people think the Earth is flat).}}
