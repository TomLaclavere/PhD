...
The complete software architecture of the qubicsoft's instrument simulation is resumed in Figure~\ref{fig:diagram_qubicsoft}
...
The CMM is very similar to FMM algorithm, and therefore, shares the same general steps. But, its programming complexity is much higher, mainly because the component separation is done alongside the maps' reconstruction. CMM holds the exact same parameters than FMM, but with additional ones : to describe the foregrounds models, to describe the fitting of the mixing matrix (Parametric or Blind reconstruction) and to handle the alternate fitting.
...
First, the check of parameters and the initialisation of all the requiered objects and variables is done in a serie of $8$ files, called \texttt{Preset}, as summarised in Figure~\ref{fig:diagram_preset} :
...
\begin{itemize}
  ...
  \item \texttt{PresetQubic} : builds the QUBIC dictionary and the \texttt{JointAcquisitionComponentsMapMaking} object, for both simulation and reconstruction.
...
The algorithm steps of the CMM are very similar with the FMM ones, we are not describing them again. The main difference stands with the alternate fitting of the mixing matrix and component maps. This extra operation is done within a loop, where both are fitted. 
...
\node[optnode] (FitGain)  at (4.5, 6)  {Update detector\\gains (optional)};
\node[decnode] (StopCond) at (0, 3)    {Converged or\\max.\ iterations ?};
\node[procnode] (FinalPCG) at (0, 0)  {Final PCG\\(extra iterations)};
\node[decnode] (DecSpec)  at (0, -3)   {Power spectra ?};
\node[optnode] (Spectra)  at (4.5, -3) {Cross angular\\power spectra};
\node[ionode]  (Store)    at (0, -6)   {Store outputs :\\TOD, maps, mixing matrix,\\spectral indices, gains, spectra};
\node[optnode] (Plots)    at (6, -6) {Automated plots};
\node[optnode] (Fit)      at (0, -9)   {MCMC to\\estimate $r$};
...
\draw[flow, dashed] (FitMM) -- (FitGain);
\draw[flow] (FitMM) -- (StopCond);
...
It should be noted that this functionality was developed during the writing of this thesis and therefore required continuous testing and refinement. Nevertheless, it has already been used in some of the cases discussed in Chapter~\ref{chap:tod}.
