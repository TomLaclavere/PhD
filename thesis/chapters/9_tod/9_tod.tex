...
The result on the effect of the coverage cut value are presented in Figure~\ref{fig:cov_cut_sigma_r}. A sweet spot is visible for coverage cut equal to $0.2$, meaning that the pixels where the coverage is below $20\%$ of the max are discarded. Below this value, $\sigma_r$ is higher because we are adding pixels with not enough information, leading to a degradation of the signal-to-noise ratio. Above $0.2$, $\sigma_r$ value is raising, which means that we are discarding too much information, and by extension, loosing information inside the patch. We are using this value for the end-to-end simulations.
...
\begin{figure}[!htbp]
    ...
    \caption{Convergence of the spectal index $\beta$. The x-axis represents the loop iteration number. (top) Convergence of the $\beta$ value. (bottom) Convergence of the residual value, the difference between the true parameter and the fitted one.}
    \label{fig:beta_convergence_n_iter_15}
\end{figure}

One would need to be careful with these values, as they do not represent an absolute rule for CMM. Indeed, they are not performed on a large statistical sample but only on one specific case. Furthermore, we can't find explanation for diverging convergence for some $N_\text{iter}$ values and not for others. An extension of this analysis may be needed in the future, or developing a non-linear solver to remove the need for the alternate convergence.
