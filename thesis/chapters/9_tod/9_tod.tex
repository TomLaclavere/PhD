...
\begin{figure}[!htbp]
    ...
    \caption{Convergence of TOD with the number of subbands, without and without interpolation.}
    \label{fig:tod_conv}
\end{figure}
...
The binning of the spectra is done for multiple purposes. The main one is to improve the signal-to-noise ratio. Indeed, when we are binning, we are averaging multiple modes into one, leading to a smaller covariance (it is divided by $\Delta_\ell$, see equation \ref{eq:xspec_noise_cov_matrix}). It also leads to a less costly fitting procedure, as we have to compute less cross-spectra value, leading to a runnable MCMC. More importantly, as we observe a small fraction of the sky, NaMaster~\cite{alonso2019} computes the spectra on a mask map. Doing so, it breaks the orthogonality of the spherical harmonics, mixing power between neighbouring modes. Therefore, averaging close modes is a way to have uncorrelated power spectra with masked sky. One can note that binning is possible only because foregrounds' models evolve smoothly in $\ell$. 

As an empirical and extensive analysis was performed on the spectra parameters by Jean-Christophe Hamilton and Louise Mousset\cite{QUBICI, QUBICII}, we decided to only test few values to compare with $\Delta_\ell = 30$. We only ran simulations for $\Delta_\ell = \left[20, 30, 40, 50\right]$. The result is visible in Figure~\ref{fig:delta_ell_sigma_r}.
...
To determine the optimal combination of these parameters, we are considering the convergence of the spectral index, which characterises the reconstruction of the mixing matrix. The analysis is performed by fixing the number of loops to $100$ and running the reconstruction for different numbers of PCG iterations per loop\footnote{Initially, we fixed the total number of PCG iterations and adjusted the number of loops and the number of iterations per loop accordingly. However, since $N_{\text{iter}}$ primarily controls the reconstruction of the component maps, whereas $N_{\text{loop}}$ governs the convergence of the mixing matrix, we found it more informative to keep one parameter fixed. Moreover, the total number of iterations is not, by itself, a meaningful criterion for assessing convergence.}. The starting point is choosen randomly at $2\sigma$ from the real value, but is kept the same for all tests. After the loop, a PCG with $150$ iterations is run to properly reconstruct the maps, which is not possible with the low $N_\text{iter}$ values used inside the loop. The number will be increased for the end-to-end simulations. 
The results are shown in Figure~\ref{fig:beta_convergence} summerised in the Table~\ref{table:niter_convergence}. In this table, the first column $N_\text{iter}$ is the number of PCG iteration per loop, the second column is the loop number where the fit of the spectral index converged, and the third one is the residual value for the spectral index (the difference between the true value and the fitted one).

\begin{figure}[!htbp]
    \begin{minipage}{0.5\linewidth}
        \centering
        \includegraphics[width=\linewidth]{convergence_beta.png}
    \end{minipage}
    \hfill
    \begin{minipage}{0.5\linewidth}
        \centering
        \includegraphics[width=\linewidth]{convergence_beta_residual.png}
    \end{minipage}
    \caption{Convergence of spectal index $\beta$ value (left) and residual (right) with the number of loop iterations.}
...
\rmk{I don't add the same plot for blind CMM, because the reconstruction was bad in all cases. Even if we can see a convergence, some mixing matrix elements were far from the true value, and the reconstructed maps look very bad.}
